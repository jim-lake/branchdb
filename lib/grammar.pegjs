...
create_table_only
  = !( create_start ( INDEX / TRIGGER / VIEW / VIRTUAL / DATABASE / SCHEMA ) ) c:( create_table )
  { return c; }

create_index_only
  = !( create_start ( TABLE / TRIGGER / VIEW / VIRTUAL / DATABASE / SCHEMA) ) c:( create_index )
  { return c; }

create_trigger_only
  = !( create_start ( TABLE / INDEX / VIEW / VIRTUAL / DATABASE / SCHEMA) ) c:( create_trigger )
  { return c; }

create_view_only
  = !( create_start ( TABLE / INDEX / TRIGGER / VIRTUAL / DATABASE / SCHEMA) ) c:( create_view )
  { return c; }

create_virtual_only
  = !( create_start ( TABLE / INDEX / TRIGGER / VIEW / DATABASE / SCHEMA) ) c:( create_virtual )
  { return c; }

create_database_only
  = !( create_start ( TABLE / INDEX / TRIGGER / VIEW / VIRTUAL / SCHEMA) ) c:( create_database )
  { return c; }

create_schema_only
  = !( create_start ( TABLE / INDEX / TRIGGER / VIEW / VIRTUAL / DATABASE ) ) c:( create_schema )
  { return c; }

create_schema "CREATE SCHEMA Statement"
 = s:(create_start) SCHEMA o id:( id_schema )
 {
  return Object.assign({
    'type': 'statement',
    'variant': 'create',
...
create_database "CREATE DATABASE Statement"
 = s:(create_start) DATABASE o id:( id_database )
 {
  return Object.assign({
    'type': 'statement',
    'variant': 'create',
...
drop_types "DROP Type"
  = t:( TABLE / INDEX / TRIGGER / VIEW / DATABASE / SCHEMA ) o
  { return keyNode(t); }
...
stmt_set "SET Statement"
  = SET o n:( id_name ) o ( TO / sym_equal ) o v:( id_name / literal_value )
  {
    return {
      type: 'statement',
      variant: 'variable',
